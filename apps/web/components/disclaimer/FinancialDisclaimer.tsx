...
export function FinancialDisclaimer({
  ...
  if (compact) {
    return (
      <div
        className={cn(
          'flex items-center gap-2 p-2.5 rounded-lg border border-border/80 bg-slate-900/40 text-[11px] text-slate-400',
          className
        )}
        {...props}
      >
        ...
        <p>
          <span className="font-semibold text-slate-300">Regulatory Notice:</span> AEGIS INVEST is an analytical research platform. It does not provide investment advisory services or guarantee financial profits or minimum losses.
        </p>
      </div>
    );
  }
